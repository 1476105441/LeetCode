...
public class No2 {
    ...
    //解法一：使用最小堆，7ms
    /*public int kthSmallest(int[][] matrix, int k) {
        int m = matrix.length,n = matrix[0].length,l = 0,size = m*n - k + 1;
    ...
    public void shift(int[] nums,int s,int n){
        int i = s,j = (i << 1)+1;
        while(j < n){
    ...
    public void update(int[] nums,int s){
        int i = s,p = (i-1) >> 1;
        while(i > 0){
            if(nums[i] < nums[p]){
                int temp = nums[i];
    ...
    //解法二：另类的二分查找，0ms
    public int kthSmallest(int[][] matrix, int k) {
        int m = matrix.length,n = matrix[0].length,l = matrix[0][0],r = matrix[m-1][n-1];
        while(l < r){
            int c = l + ((r-l) >> 1);
            if(count(matrix,c) < k){
                l = c+1;
    ...
    public int count(int[][] matrix,int val){
        int res = 0,m = matrix.length,n = matrix[0].length,i = m-1,j = 0;
        while(i >= 0 && j < n){
            if(val >= matrix[i][j]){
                res += i+1;
                j++;
